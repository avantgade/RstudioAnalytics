---
title: "ISLR Tidymodels Labs - 6 - Linear Model Selection and Regularization"
output: html_document
---

# Linear Model Selection and Regularization

This lab will take a look at the regularization models and hyperparameter tuning. These models are related to the models we saw in chapter 3 and 4, with the difference that they contain a regularization term.
...
ridge_spec <- linear_reg(mixture = 0, penalty = 0) %>%
  set_mode("regression") %>%
  set_engine("glmnet") #setting mixture to 0 for ridge model regression, using gmlnet engine
```

Once the specification is created we can fit it to our data. We will use all the predictors.

```{r}
ridge_fit <- fit(ridge_spec, Salary ~ ., data = Hitters) #utilizing all predictors on Hitters dataset for model fitting
```
...
```{r}
tidy(ridge_fit) #penalty parameter set to 0
```

Let us instead see what the estimates would be if the penalty was 11498.

```{r}
tidy(ridge_fit, penalty = 11498) #penalty parameter set much higher, note the change in values.
```
...
tidy(ridge_fit, penalty = 50) #estimates increase as penalty increases.
```

We can visualize how the magnitude of the coefficients are being regularized towards zero as the penalty goes up. 

```{r}
plot(ridge_fit$fit, xvar = "lambda") #visualization of penalties
```
...
```{r}
Hitters_split <- initial_split(Hitters, strata = "Salary") #beginning validation split
Hitters_train <- training(Hitters_split) #training sample
Hitters_test <- testing(Hitters_split) #test sample
Hitters_fold <- vfold_cv(Hitters_train, v = 10) #folds
```
...
ridge_recipe <- 
  ...
  step_normalize(all_predictors()) #normalizing data and collecting predictors 
```
...
```{r}
ridge_spec <- 
  linear_reg(penalty = tune(), mixture = 0) %>% #tuning penalty value
  set_mode("regression") %>% 
  set_engine("glmnet")
```
...
ridge_workflow <- workflow() %>% 
  add_recipe(ridge_recipe) %>% #combining ridge model with tuned penalty model with normalized data predictors into workflow
...
```{r}
penalty_grid <- grid_regular(penalty(range = c(-5, 5)), levels = 50) #creating various iterations of penalty
penalty_grid
```
...
```{r}
tune_res <- tune_grid(
  ridge_workflow, #calling workflow
  resamples = Hitters_fold,#utilizing folded data
  grid = penalty_grid #utilizing the 50 penalty values
)
tune_res
```
...
```{r}
autoplot(tune_res) #plotting tune for visualization
```
...
```{r}
collect_metrics(tune_res) #collecting metrics of various values based on penalty value and config
```
...
```{r}
best_penalty <- select_best(tune_res, metric = "rsq") #Note: in People Analytics, it is usually a best practice to use RMSE instead of rsq due to the ease of interpretability by a layperson
best_penalty #selecting optimum presets
```
```{r}
best_penalty_rmse <- select_best(tune_res, metric = "rmse") #Notice how a different penalty value is assigned "best". Know your metrics, when do use them, why to use them, and for which audiance to use them!
best_penalty_rmse #again identifying optimal presets cept with rmse
```
...
```{r}
ridge_final <- finalize_workflow(ridge_workflow, best_penalty) #updating workflow with ideal penalty preset
ridge_final_fit <- fit(ridge_final, data = Hitters_train) #creating model function based off training dataset
```

This final model can now be applied on our testing data set to validate the performance

```{r}
augment(ridge_final_fit, new_data = Hitters_test) %>% #augmenting to make model function work on test data set
  rsq(truth = Salary, estimate = .pred) #accuracy
```
...
```{r}
lasso_recipe <- 
  recipe(formula = Salary ~ ., data = Hitters_train) %>% #creating recipe with all predictors from hitters training dataset
  step_novel(all_nominal_predictors()) %>% 
  step_dummy(all_nominal_predictors()) %>% 
  step_zv(all_predictors()) %>% 
  step_normalize(all_predictors()) #normalizing
```
...
```{r}
lasso_spec <- 
  linear_reg(penalty = tune(), mixture = 1) %>% #creating function to create lasso regression using glmnet engine with tuned penalty
  set_mode("regression") %>% 
  set_engine("glmnet") 
...
```{r}
penalty_grid <- grid_regular(penalty(range = c(-2, 2)), levels = 50) #same penalty process as above, 50 levels but with different range
```
...
tune_res <- tune_grid(
  lasso_workflow,
  resamples = Hitters_fold, #tuning grid
  grid = penalty_grid
)
...
best_penalty #best rsq specs
```
```{r}
best_penalty_rmse <- select_best(tune_res, metric = "rmse") #Notice how a different penalty value is assigned "best". Know your metrics, when do use them, why to use them, and for which audiance to use them!
best_penalty_rmse #best rmse specs
```
...
lasso_final_fit <- fit(lasso_final, data = Hitters_train) #using training dataset
```
...
```{r}
augment(ridge_final_fit, new_data = Hitters_test) %>% #using test dataset
...
lm_spec <- 
  linear_reg() %>% 
  set_mode("regression") %>%  #setting up linear regression
  set_engine("lm")
```
...
pca_recipe <- 
  ...
  step_dummy(all_nominal_predictors()) %>% 
  step_zv(all_predictors()) %>% 
  step_normalize(all_predictors()) %>% #similar recipe steps up until...
  step_pca(all_predictors(), threshold = tune()) #this. tune represents a hyperparameter tbd.
pca_workflow <- 
  workflow() %>% 
  add_recipe(pca_recipe) %>% 
  add_model(lm_spec) #creating workflow with above components
```

We create a smaller grid for `threshold` and we don't need to modify the range since `[0, 1]` is an acceptable range.

```{r}
threshold_grid <- grid_regular(threshold(), levels = 10) #range left as default 0 - 1
threshold_grid
```
...
tune_res <- tune_grid(
  ...
  grid = threshold_grid #creating grid with 100 fits
)
```
...
```{r}
autoplot(tune_res) #visualizing
```

But we can still select the best model.

```{r}
best_threshold <- select_best(tune_res, metric = "rmse") #creating best parameter
```
...
pca_final_fit <- fit(pca_final, data = Hitters_train) #fit function
```
...
```{r}
#From: http://mixomics.org/install/
...
BiocManager::install("mixOmics") #previous github install code not working, pulled this from bioc website
```
...
pls_recipe <- 
  ...
  step_pls(all_predictors(), num_comp = tune(), outcome = "Salary") #same stuff as above with outcome clearly defined

lm_spec <- linear_reg() %>% 
  set_mode("regression") %>% 
  set_engine("lm") #same stuff as above

pls_workflow <- workflow() %>% 
  add_recipe(pls_recipe) %>% 
  add_model(lm_spec) #same stuff as above

num_comp_grid <- grid_regular(num_comp(c(1, 20)), levels = 10) #the definition of insanity is doing the same thing over and over again

tune_res <- tune_grid(
  pls_workflow,
  resamples = Hitters_fold, 
  grid = num_comp_grid #insanity (i know this is a different process but I couldnt resist. It's the little easter eggs for yourself that make you content while you code)
)
...
```{r}
pls_final_fit #calculating partial least squares
```
