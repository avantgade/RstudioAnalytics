---
title: "ISLR Tidymodels Labs - 3 - Linear Regression"
...
Without selecting the specific columns, it will simply bind the `.pred` and `.resid` to the left side of the data.

```{r}
augment(lm_fit, new_data = Boston)
```

If you want to focus specifically on the median value and the `.pred`, then you can select those two columns
