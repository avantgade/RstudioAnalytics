...
Comments_df <- Data %>%
    select(c(ID, cons)) %>%
    filter(!is.na(cons)) %>%
    rename('comments' = 'cons') #collecting pro comments
...
size_comments <- as.data.frame(Comments_df[grep(size_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Comments_df %>%
    mutate(size = ifelse(comments %in% size_comments$comments, "Y",
                             "N")) #Creating a new object Cons from Comments_df. If any of the comments in the "comments" column match (%in%) the comments exactly, they get a "Y". If not they get a "N" in the new "benefits" column
```
...
ethics_comments <- as.data.frame(Comments_df[grep(ethics_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(ethics = ifelse(comments %in% ethics_comments$comments, "Y",
                             "N")) #Adding new column "Ethics" to Cons.
```
...
safety_comments <- as.data.frame(Comments_df[grep(safety_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(safety = ifelse(comments %in% safety_comments$comments, "Y",
                             "N")) #Adding new column "safety" to Cons.
```
...
corporate_comments <- as.data.frame(Comments_df[grep(corporate_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(corporate = ifelse(comments %in% corporate_comments$comments, "Y",
                             "N")) #Adding new column "corporate" to Cons.
```
...
brand_comments <- as.data.frame(Comments_df[grep(brand_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(brand = ifelse(comments %in% brand_comments$comments, "Y",
                             "N")) #Adding new column "brand" to Cons.
```
...
stock_comments <- as.data.frame(Comments_df[grep(stock_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(stock = ifelse(comments %in% stock_comments$comments, "Y",
                             "N")) #Adding new column "stock" to Cons.
```
...
innovation_comments <- as.data.frame(Comments_df[grep(innovation_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(innovation = ifelse(comments %in% innovation_comments$comments, "Y",
                             "N")) #Adding new column "innovation" to Cons.
```
...
remote_comments <- as.data.frame(Comments_df[grep(remote_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(remote = ifelse(comments %in% remote_comments$comments, "Y",
                             "N")) #Adding new column "remote" to Cons.
```
...
stable_comments <- as.data.frame(Comments_df[grep(stable_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(stable = ifelse(comments %in% stable_comments$comments, "Y",
                             "N")) #Adding new column "stable" to Cons.
```
...
location_comments <- as.data.frame(Comments_df[grep(location_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(location = ifelse(comments %in% location_comments$comments, "Y",
                             "N")) #Adding new column "location" to Cons.
```
...
opmodel_comments <- as.data.frame(Comments_df[grep(opmodel_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(opmodel = ifelse(comments %in% opmodel_comments$comments, "Y",
                             "N")) #Adding new column "opmodel" to Cons.
```
...
community_comments <- as.data.frame(Comments_df[grep(community_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(community = ifelse(comments %in% community_comments$comments, "Y",
                             "N")) #Adding new column "community" to Cons.
```
...
contracts_comments <- as.data.frame(Comments_df[grep(contracts_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(contracts = ifelse(comments %in% contracts_comments$comments, "Y",
                             "N")) #Adding new column "contracts" to Cons.
```
...
challenge_comments <- as.data.frame(Comments_df[grep(challenge_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(challenge = ifelse(comments %in% challenge_comments$comments, "Y",
                             "N")) #Adding new column "challenge" to Cons.
```
...
worklife_comments <- as.data.frame(Comments_df[grep(worklife_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(worklife = ifelse(comments %in% worklife_comments$comments, "Y",
                             "N")) #Adding new column "worklife" to Cons.
```
...
culture_comments <- as.data.frame(Comments_df[grep(culture_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(culture = ifelse(comments %in% culture_comments$comments, "Y",
                             "N")) #Adding new column "culture" to Cons.
```
...
flexible_comments <- as.data.frame(Comments_df[grep(flexible_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(flexible = ifelse(comments %in% flexible_comments$comments, "Y",
                             "N")) #Adding new column "flexible" to Cons.
```
...
experience_comments <- as.data.frame(Comments_df[grep(experience_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(experience = ifelse(comments %in% experience_comments$comments, "Y",
                             "N")) #Adding new column "experience" to Cons.
```
...
advancement_comments <- as.data.frame(Comments_df[grep(advancement_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(advancement = ifelse(comments %in% advancement_comments$comments, "Y",
                             "N")) #Adding new column "advancement" to Cons.
```
...
learn_comments <- as.data.frame(Comments_df[grep(learn_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(learn = ifelse(comments %in% learn_comments$comments, "Y",
                             "N")) #Adding new column "learn" to Cons.
```
...
manage_comments <- as.data.frame(Comments_df[grep(manage_pattern, Comments_df$comments, value = FALSE, perl = TRUE),]) # Searching over the cons comments to find corresponding patterns.

Cons <- Cons %>%
    mutate(manage = ifelse(comments %in% manage_comments$comments, "Y",
                             "N")) #Adding new column "manage" to Cons.
```


#Writing Cons to Excel
...
addWorksheet(wb, "Cons Comment Report") #naming spreadsheet
...
writeData(wb, "Cons Comment Report", Cons, startRow = 8) #put your DF (in this case TEST) into the sheet under your writing (row 8)

hs1 <- createStyle(textDecoration = "Bold") #create a new style for heading

addStyle(wb, style = hs1, rows = 8, cols = 1:nrow(Cons), sheet = "Cons Comment Report") #Tell it where to go. We'll do 50 columns in this case so it can grow if needed
...
addFilter(wb, "Cons Comment Report", row = 8, cols = 1:nrow(Cons)) #Add your filter as well. If you're trying to impress, you might as well go all in :)
...
saveWorkbook(wb, paste0("C:/Advanced Analytics Project/00_Data/Cons_Comment_Report_", format(floor_date(Sys.Date()-months(1), "month"), "%B_%Y") , ".xlsx"), overwrite = TRUE)
```
